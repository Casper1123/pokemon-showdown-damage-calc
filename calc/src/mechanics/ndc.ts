...
export function calculateNDC(
  ...
  if (move.named('Photon Geyser', 'Light That Burns the Sky', 'Neutron Ray') ||
  ...
  if ((move.named('Sky Drop') &&
    ...
    (move.named('Needle Throw') && field.isGravity)
  ) {
    return result;
  }
...
export function calculateBasePowerNDC(
  ...
  // region NDC modded
  case 'Desert Song':
    basePower = move.bp * (field.hasWeather('Sand') ? 1.5 : 1);
    desc.moveBP = basePower;
    break;
  case 'Stalk':
